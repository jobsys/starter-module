						...
						<a-dropdown>
							...
							<template #overlay>
								<a-menu>
									...
									<a-menu-item key="course" v-if="$auth('api.manager.course.edit')">
										<Link :href="route('page.manager.course', { create: 1 })">
											<NewbieIcon icon="iconfont-team-line"></NewbieIcon>
											新增开班
										</Link>
									</a-menu-item>

									<a-menu-item key="client" v-if="$auth('api.manager.client.edit')">
										<Link :href="route('page.manager.client', { create: 1 })">
											<NewbieIcon icon="iconfont-user--line"></NewbieIcon>
											新增委托单位
										</Link>
									</a-menu-item>

									<a-menu-item key="partner" v-if="$auth('api.manager.partner.edit')">
										<Link :href="route('page.manager.partner', { create: 1 })">
											<NewbieIcon icon="UsergroupAddOutlined"></NewbieIcon>
											新增合作单位
										</Link>
									</a-menu-item>

									<a-menu-item key="teacher" v-if="$auth('api.manager.teacher.edit')">
										<Link :href="route('page.manager.teacher', { create: 1 })">
											<NewbieIcon icon="iconfont-account-pin-box-line"></NewbieIcon>
											新增教师
										</Link>
									</a-menu-item>

									<a-menu-item key="subject" v-if="$auth('api.manager.subject.edit')">
										<Link :href="route('page.manager.subject', { create: 1 })">
											<NewbieIcon icon="iconfont-book--line"></NewbieIcon>
											新增课程
										</Link>
									</a-menu-item>

									<a-menu-item key="post" v-if="$auth('api.manager.post.edit')">
										<Link :href="route('page.manager.post.index', { create: 1 })">
											<NewbieIcon icon="iconfont-newspaper-line"></NewbieIcon>
											新增新闻资讯
										</Link>
									</a-menu-item>
								</a-menu>
...
import { useMediaStore, useNotificationStore, useUserStore } from "@manager/stores"
...
useFetch()
	.get(route("api.manager.media-library.config"))
	.then((res) => {
		mediaStore.setCategories(res.result.categories)
		mediaStore.setConfig({
			uploadUrl: route("api.manager.media-library.upload"),
			fetchUrl: route("api.manager.media-library.items"),
			itemUrl: route("api.manager.media-library.item"),
			deleteUrl: route("api.manager.media-library.delete"),
		})
	})
</script>
<style lang="less">
