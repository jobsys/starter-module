<template>
	<component :is="root" ref="editorRef"></component>
</template>
...
import { cloneDeep, debounce, isArray, isFunction, isString, isUndefined } from "lodash-es"
import { useFormFormat, formLabel, useProcessStatusSuccess, useFormFail } from "@/lts/hooks/core/form"
import { getFullCode, getOptionsValue } from "@/lts/utils"
import { useModalConfirm, useFetch, useModalSuccess, useDateType } from "@/lts/hooks/core"
...
let editorRef = ref(null) //总的容器
let root = ref(null) //容器组件
let richEditors = {} //富文本ref
let uploadRefs = {} //上传组件ref
...
const fetchItem = () => {
	...
	if (props.fetchUrl) {
		useFetch()
			.get(props.fetchUrl, { params: props.fetchData })
			.then((res) => {
				state.isInit = false
	...
	formData.value.forEach((item) => {
		...
		if (item.type === "switch") {
			...
			if (item.defaultProps && item.defaultProps.type === "range") {
				value = value ? [useDateType(value[0]), useDateType(value[1])] : []
			} else {
				value = value ? useDateType(value) : ""
			}
			...
			setTimeout(
				(value) => {
					uploadRefs["uploader" + item.key] && uploadRefs["uploader" + item.key].component.exposed.updateValue(value)
				},
				0,
				value,
			)
...
const submit = () => {
	editorRef.value
		.validate()
		.then(async () => {
			let form = useFormFormat(state.submitForm)
			formData.value.forEach((item) => {
				switch (item.type) {
					case "address":
			...
			if (props.processSubmitData) {
				form = await props.processSubmitData(form, state.submitForm)
				if (form === false) {
					return
				}
			}
			...
			if (props.onSuccess) {
				...
				useProcessStatusSuccess(res, () => {
					useModalSuccess("提交成功", props.reloadName, () => {
...
const _createFormItem = (item, labelWidth, isHalf) => {
	...
	return returnItem
		? createElement(Form.Item, itemProps, {
			default() {
...
const _createTips = (item) => {
	return item.tips
		? createElement("div", Object.assign({ class: "tips" }, item.tipsProps || {}), {
			default() {
...
const _createInput = (item) => {
	if (state.submitForm[item.key] === null || state.submitForm[item.key] === "null" || state.submitForm[item.key] === undefined) {
	...
	return createElement(
		...
		Object.assign(
			{
				value: state.submitForm[item.key],
...
const _createSelect = (item) => {
	...
	return createElement(
		...
		Object.assign(
			{
				value: state.submitForm[item.key] || undefined,
				options: options,
				disabled: item.disabled,
				allowClear: true,
		...
		Object.assign(
			{
				...
				allowClear: true,
				filterOption: false,
				notFoundContent: state.temporary[loadKey].loading ? undefined : "无内容",
				class: item.class || "",
...
const _createUploader = (item) => {
	...
	uploadRefs["uploader" + item.key] = element
	return element
}
...
defineExpose({ getForm, getSubmitForm, setForm, reset, doInit })
</script>
