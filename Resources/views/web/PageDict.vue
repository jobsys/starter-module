<template>
	<NewbieTable ref="list" :url="route('api.manager.starter.dict.items')" :columns="columns()" row-key="id">
		<template #functional>
			<NewbieButton type="primary" icon="PlusOutlined" @click="onEdit(false)">新增字典 </NewbieButton>
		</template>
	</NewbieTable>
	...
	<NewbieModal type="drawer" v-model:visible="state.showItemDrawer" title="字典项列表" :width="1000">
		<NewbieTable
			...
			:page="false"
			:filter-form="false"
			:fetched="(res) => ({ items: res.result })"
			:columns="itemColumns()"
			row-key="id"
		>
			<template #functional>
				<NewbieButton type="primary" icon="PlusOutlined" @click="onEditItem(false)">新增字典项 </NewbieButton>
			</template>
		</NewbieTable>
	</NewbieModal>
	...
	<NewbieModal v-model:visible="state.showItemEditorModal" title="字典项编辑" :width="600">
		<NewbieEdit
			...
			:form="getItemForm()"
			:on-close="closeItemEditorModal"
			:process-submit-data="onBeforeSubmitItem"
			@after-success="closeItemEditorModal(true)"
		/>
	</NewbieModal>
</template>
...
const onBeforeSubmitItem = ({ processedForm }) => {
	processedForm.dictionary_id = state.currentDictItem.id
