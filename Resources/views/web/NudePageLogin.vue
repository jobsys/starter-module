				...
				<a-form ref="form" :model="state.loginForm" :rules="state.loginFormRules">
					<div class="px-8 py-4 md:py-4">
						...
						<a-form-item name="captcha">
							<a-input v-model:value="state.loginForm.captcha" placeholder="请计算右侧算式答案" size="large">
								<template #prefix>
									<NewbieIcon icon="CalculatorOutlined" />
								</template>
								<template #addonAfter>
									<img
										class="captcha_code"
					...
					<div class="px-10 py-6 bg-gray-100 border-t border-gray-100 flex justify-center items-center">
						<a-form-item style="margin-bottom: 0">
							<NewbieButton :fetcher="state.loginFetcher" type="primary" :config="{ size: 'large' }" block @click="doLogin"
								>登录
							</NewbieButton>
						</a-form-item>
					</div>
				...
				<div class="py-3" v-if="state.roleOptions && state.roleOptions.length">
					...
					>
						<div class="rounded p-2 shadow bg-emerald-600 text-white">
							<NewbieIcon icon="UserOutlined" :style="{ fontSize: '30px' }" />
						</div>
						<div class="ml-2">
							{{ option.label }}
						</div>
...
import { NewbieIcon, NewbieButton } from "@web/components"
import { encryptBySM3 } from "@/js/utils/cipher"
import Logo from "@public/images/logo.png"
import { useFetch } from "@/js/hooks/web/network"
import { useProcessStatus } from "@/js/hooks/web/form"
...
const doLogin = () => {
	form.value.validate().then(async () => {
		try {
			...
			data.password = encryptBySM3(data.password)
